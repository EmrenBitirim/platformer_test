...
function Coin.updateAll()
    for i, instance in ipairs(ActiveCoins) do -- i = index, instance = element, ipairs(table)
        instance:update(dt)
    end 
end 
...
function Coin:jiggle()
    local jiggleOffsetX = math.sin(love.timer.getTime() * 6) / 10
    local jiggleOffsetY = math.cos(love.timer.getTime() * 6) / 10
        for i, instance in ipairs(ActiveCoins) do
